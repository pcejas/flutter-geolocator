...
class Geolocator {
  ...
  Future<Position> getCurrentPosition({LocationAccuracy desiredAccuracy = LocationAccuracy.best}) async {
    ...
    if (permission == PermissionStatus.granted) {
      final LocationOptions locationOptions =
          LocationOptions(accuracy: desiredAccuracy, distanceFilter: 0, forceAndroidLocationManager: await _shouldForceAndroidLocationManager());
  ...
  Stream<Position> getPositionStream([LocationOptions locationOptions = const LocationOptions()]) async* {
    ...
    if (permission == PermissionStatus.granted) {
      _onPositionChanged ??=
          _eventChannel.receiveBroadcastStream(Codec.encodeLocationOptions(locationOptions)).map<Position>((dynamic element) => Position._fromMap(element.cast<String, dynamic>()));
